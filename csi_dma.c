...
error_t csi_dma_config_no_alloc_mem(csi_dma_config_t *cdma, uint32_t dma_size, uint32_t MemAddress)
{
	int i;
	...
	g_host.dma_buf_saddr = (void*)MemAddress;

	for(i = 0; i < 4; i++)
	{
		cdma->dma[i].buffer.size = dma_size;
		cdma->dma[i].buffer.vir_addr = (void*)MemAddress;
		cdma->dma[i].buffer.phy_addr = (void*)MemAddress;
		cdma->dma[i].length = dma_size;
	}
